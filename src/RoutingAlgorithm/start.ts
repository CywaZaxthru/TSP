async function iniateFindingRoute(locationStringArray: String[]){

    let costArray: number[][]  = new Array(locationStringArray.length).fill(0).map(() => new Array(locationStringArray.length));
    
    //Filling Array with cost of Routes
    costArray = await fillSquare(costArray, locationStringArray);
    //calculating Route
    let route: number[] = calculateRoute(costArray);

    return route;
}

function calculateRoute(weightMatrix: number[][]): number[]{
    let fullPath: number[];
...
async function fillSquare(emptySquare: any[][], addressArray: String[]){
    let promises = [];
    for(let i: number = 0; i<emptySquare.length; i++){
        for(let j: number = i+1; j<emptySquare[0].length; j++){
            if(!emptySquare[i][j]){
    ...
    for(let i: number = 0; i<emptySquare.length; i++){
        for(let j: number = 0; j<emptySquare.length; j++){
